...
function cavani_tm_growth_popup(){
	
	"use strict";
	var modalBox		= jQuery('.cavani_tm_modalbox');
	var button			= jQuery('.cavani_tm_growth .news_list > ul > li .post_title h3 a');
	var closePopup		= modalBox.find('.close');
...
function check_for_hash(){
	...
	if(["#growth", "#about", "#travel", "#food", "#contact"].includes(hash)) {
		// hash = hash.replace('#', '');
		console.log(hash);
